...
public class OrderController {
    ...
    @ResponseBody
    public ResultWithPage getAllWithPage(@RequestParam(value = "page", required = false) String currentPage, @RequestParam(value = "rows", required = false) String rows, Order order) {
        ...
        if (EmptyUtil.isStrEmpty(order.getGoodsName()) && EmptyUtil.isStrEmpty(order.getCode()) && EmptyUtil.isStrEmpty(order.getState())) {
            list = orderService.findAll();
        } else {
            list = orderService.findByLike(order);
        }
        //用PageInfo对结果进行包装
        PageInfo<Order> pageInfo = new PageInfo<>(list);

        Iterator<Order> userIterator = pageInfo.getList().iterator();
    ...
    @ResponseBody
    public Map<String, String> addObject(@RequestBody Order order) {
    ...
    @ResponseBody
    public Map<String, String> updateObject(@RequestBody Order order) {
        Map<String, String> map = new HashMap<>();
